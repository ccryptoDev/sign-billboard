...
<link href="/assets/plugins/custom/datatables/datatables.bundle.css" rel="stylesheet" type="text/css"/>
                                    ...
                                    <tbody>
                                        @foreach($invoices as $key => $val)
                                            ...
                                            if($val->campaign_id == 0){
                                                ...
                                                $business_check = isset($extra['business_check'])?$extra['business_check']:'';
                                            }
                                            ?>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php
                                                    if($val->paid_at != null) {
                                                        echo date('m-d-Y H:i:s', $val->paid_at);
                                                    }
                                                    else if($val->pay_date != null){
                                                        $created = date_create($val->pay_date);
                                                        echo date_format($created, "m-d-Y H:i:s");
                                                    }
                                                    ?>
                                                </td>
                                                <th>{{$business_check}}</th>
                                                <td>${{number_format($val->part_amount,2)}}</td>
                                                <td>
                                                    <?php
                                                        $payment_type = 0;
                                                        if($val->ach == null){
                                                            $payment_type = $manual_invoice==true?1:0;
                                                        }
                                                        else{
                                                            // $payment_type = $val->ach==1?1:0;
                                                            $payment_type = $val->ach;
                                                        }
                                                    ?>
                                                    @if(session('level') == 2)
                                                    <select class="form-control payment_type" data-id="{{$val->id}}" autocomplete="off">
                                                        <option value="2" {{$payment_type==2?'selected':''}}>CC</option>
                                                        <option value="1" {{$payment_type==1?'selected':''}}>ACH</option>
                                                        <option value="0" {{$payment_type==0?'selected':''}}>Check</option>
                                                    </select>
                                                    @else
                                                        @if($payment_type == 0)
                                                            <span class="label label-xl label-danger label-inline mr-2">Check</span>
                                                        @elseif($payment_type == 1)
                                                            <span class="label label-xl label-success label-inline mr-2">ACH</span>
                                                        @else
                                                            <span class="label label-xl label-success label-inline mr-2">CC</span>
                                                        @endif
                                                    @endif
                                                </td>
                                                <td>
                                                    ${{number_format($val->part_amount,2)}}
                                                </td>
                                                <td>
                                                    <?php
                                                        $paid = false;
                                                ...
                                                <td>
                                                    <select class="form-control status" data-id="{{$val->id}}" data-sub="{{$val->sub_inv_id}}" autocomplete="off">
                                                        @if($manual_invoice == false)
                                                            <option value=""></option>
                                                            <option value="0" {{$free_plan!=0&&$val->status==0?'selected':''}}>Unpaid</option>
                                                ...
                                                <td style="min-width : 150px">
                                                    ...
                                                    <a onclick="send_invoice('{{$val->id}}', '{{$val->sub_inv_id}}')" data-container="body" data-toggle="tooltip" data-placement="bottom" title="Send Email" style="cursor:pointer">
                                                    ...
                                                    @if(session('level') == 2)
                                                    <?php
                                                    $inv_id = $val->sub_inv_id==null?$val->id:$val->sub_inv_id;
                                                    $sub_flag = $val->sub_inv_id==null?0:1;
                                                    ?>
                                                    <a data-container="body" style="cursor:pointer" onclick="delete_inv('{{$inv_id}}', '{{$sub_flag}}')" data-toggle="tooltip" data-placement="bottom" title="Delete Invoice">
    ...
    <div class="modal fade" id="paymentModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        ...
        <script>
                ...
                $.ajax({
                    ...
                    success : function(res){
                        ...
                        if(res['success'] == true){
                            ...
                            $("#paymentModal input[name=business_check]").val(data['extra']['business_check'])
            ...
            if(session('level') == 2){
            ?>
            function delete_inv(id, sub_flag){
                var inv_id = id;
                var sub_flag = sub_flag;
                Swal.fire({
                    title: "Are you sure?",
                    ...
                    if (result.value) {
                        $.ajax({
                            url : '/delete-invoice/'+inv_id+'/'+sub_flag,
                            type : 'GET',
                            success : function(res){
                                if(res == 'success'){
                                    location.reload();
            ...
            <?php }?>
            function send_invoice(id, subId){
                KTApp.blockPage();
                $.ajax({
                    url : "/send-invoice",
                    type : "POST",
            ...
            $(document).ready(function(){
                var $sel_type = $('#kt_datatable').on('change','.payment_type',function (event) {
