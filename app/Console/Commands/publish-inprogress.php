    ...
    public function handle()
    {
        ...
        foreach ($campaigns as $key => $campaign) {
            if($campaign->business_name != '1 Demo' && $campaign->start_date <= $today && $campaign->end_date >= $today && ( $campaign->free_plan == 3 || $campaign->status ==1 || ($campaign->invoice_date > $today && $campaign->status == 0))){
                ...
                if(!in_array($campaign->business_name, $business_names)){
                    $business_names[] = $campaign->business_name;
                }
                foreach($ava_locations as $k => $loc){
                    $location = DB::table('tbl_locations')
                        ->where('id', $loc)->first();
                    if(!in_array($campaign->business_name."-".$location->name, $locations_by_business)){
                        for($i = 0; $i < $ava_slots[$k]; $i++){
                            // Already Exists
                            $p = $i + 1;
                            $exist = DB::table('tbl_sub_list')
                                ->where('business_name', $campaign->business_name)
                                ->where('location', $location->name)
                                ->where('camp_id', $campaign->id)
                                ->where('slots', $p)
                                ->first();
                            if(!isset($exist->id)){
                                $sub_name = "Sub-".$campaign->business_name."-".$location->name."-".$campaign->id."-".$p;
                                $locations_by_business[] = $sub_name;
                                $business_location[] = $campaign->business_name."-".$location->name;
        ...
        foreach($ads as $key => $val){
            ...
            foreach($locations as $temp){
                $exists[] = $business_name."-".$temp;
            }
        }
        foreach($locations_by_business as $key => $temp){
            if(!in_array($business_location[$key], $exist)){
                unset($locations_by_business[$key]);
            }
        }
        print_r($locations_by_business);
